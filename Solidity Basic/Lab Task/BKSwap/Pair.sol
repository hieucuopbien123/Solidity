...
contract Pair is IPair, LPToken{
    ...
    uint feeParams = 0;
    //token đã được sắp xếp khi vào đây
    ...
    function _addLiquidity(uint addingAmount1, uint addingAmount2, address to) internal returns(uint){
        ...
        */
        ...
        //hàm này ai cũng gọi được nên 1 thg bất kỳ có thể vào contract và điền bừa addingAmount dù chưa gửi gì cả
        //nguy hiểm các hàm public ai cũng gọi được nên lấy số dư như này cho chuẩn
    ...
    function addLiquidity(address to) external override returns(uint){
        ...
        //các contract được xử lý với tốc độ rất cao như C++ v và nó thực hiện đồng bộ nên k sợ 1 người chuyển vào 
        //cập nhập số dư xong 1 ông khác cũng chuyển vào thì số dư bị lỗi-> nó đồng bộ theo thứ tự hết
    ...
    function removeLiquidity(address to) external override returns(uint, uint){
        //nhận vào lượng LP token và tính ra phần gửi lại cho trader. Người ta gọi được hàm remove thì chắc chắn đã từng add trước
        //đó r. Nhận vào amount Lp token, address của nơi cần gửi tới
        /*mintFeeForDev
    ...
    //này tức là contract này phải là thứ gọi bằng call thì msg.sender sẽ là contract này và gửi 
    //vào địa chỉ nào
