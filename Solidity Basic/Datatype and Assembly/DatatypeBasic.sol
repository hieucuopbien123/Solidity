...
contract Test{
    //ufixed public fixVar = 1;//k được dùng fixed là abort Ct
    int[3] public tes1t = [-11,2,3]; // k thể dùng push, buộc có số âm bên trong
    ...
    //truyền vào bytes thì deploy xong ta truyền vào kiểu bytesN nào cx đc. VD truyền được 0x20, còn 0x200 là sai
    function getVal2(bytes memory data) public pure returns(bytes memory){
        return data;
    } 
    
    //convert từ bytes sang bytes32 -> dùng assembly vì từ mảng động sang tĩnh thì k đơn giản
    ...
    //convert string to bytes32: string -> bytes -> bytes32
