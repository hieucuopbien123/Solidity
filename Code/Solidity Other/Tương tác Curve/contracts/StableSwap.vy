# @version 0.2.4
# Ta chỉ quan tâm đến hàm exchange và get_y
...
#refer: https://www.youtube.com/watch?v=jAhKbxoeskQ&list=PLO5VPQH6OWdX-Rh7RonjZhOd9pb9zOnHW&index=9
...
def get_y(i: int128, j: int128, x: uint256, xp_: uint256[N_COINS]) -> uint256:
    ...
    #check các params đầu vào hợp lệ
    ...
    #set các param, amp và ann với n là N_COINS
    amp: uint256 = self._A() # A * (n ** (n - 1))
    # get_D giống hệt get_y về cơ chế
    D: uint256 = self.get_D(xp_, amp)
    c: uint256 = D
    S_: uint256 = 0
    Ann: uint256 = amp * N_COINS # A * (n ** n)

    #chạy 1 loop các coin X, Y, Z tương ứng 1,2,3
    _x: uint256 = 0
    ...
    # _x | S     | c | 
    # ------------------------------------------
    # X  | X     | D^2 / (X * n) -> vì _i == i == 1
    # Y  | X     | D^2 / (X * n) -> vì rơi vào else do _i = 2 còn j = 2
    # Z  | X + Z | D^3 / (X * Z * n^2) -> vì rơi vào TH _i!=j vì _i = 3 còn j = 2 
    #_i là giá trị tăng dần lên, i là token ta cần bán là 1, j là token ta cần mua là 2, Z chả làm gì là 3
    ...
    for _i in range(255):
        y_prev = y
        y = (y*y + c) / (2 * y + b - D)
        # (y^2 + D^4 / (A * n^n * X * Z * n^3)) / (2 * y + X + Z + D / (A * n^n) - D) = PT giải y sau khi rút gọn
        # Equality with the precision of 1
        if y > y_prev:
            if y - y_prev <= 1:
                break
            ...
            if y_prev - y <= 1:
                break
    return y
    # giải ra y bằng newton method. Newton method dùng cho y(n+1)=y(n)-g(Y)/g'(y) thì giai được y bằng newton method
    # nếu g(Y)=0 => cái PT của curve ta chuyển hết sang vế phải để nó = g(y) = 0 và dùng newton rút gọn ra phương trình
    # tìm y và dùng vòng for kia để tính đó
...
# hàm exchange của Curve cơ chế 
@external
@nonreentrant('lock')
def exchange(i: int128, j: int128, dx: uint256, min_dy: uint256):
    # [X,Y,Z] trong pool giả sử ta đang cần trade X sang Y
    ...
    # mấy cái bên trên handling các thứ khác. Đầu tiên là nó gửi token X vào pool là contract này thông qua hàm raw_call
    # như dưới. Nó dùng rawcall vì cái hàm transferFrom nó k giống nhau ở các ERC20 token vì có hàm return bool, có 
    # hàm chả return gì cả 
    # "safeTransferFrom" which works for ERC20s which return bool or not
    _response: Bytes[32] = raw_call(
        input_coin,
        concat(
    ...
    # nếu hàm k return thì k sao nhưng nếu return thì ta convert sang bool và assert check transfer thành công
    if len(_response) > 0:
        assert convert(_response, bool)  # dev: failed transfer
    ...
    # tính x là lượng token mới trong pool sau khi ta thêm vào = lượng token cũ xp[i] cộng với lượng ta gửi vào
    x: uint256 = xp[i] + dx_w_fee * rates[i] / PRECISION
    y: uint256 = self.get_y(i, j, x, xp)
    # tính y là lượng token ta mua, nhưng dy bên dưới mới là lượng token thực tế ta get
    ...
    # lại transfer tiếp lượng Y token cho ta bằng raw_call tương tự như TH trên
