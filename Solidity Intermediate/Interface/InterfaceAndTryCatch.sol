...
//nếu contract đó ở cùng file như này. Khi đó ta phải cho contract cần gọi kế thừa cái interface tự tạo ra. Còn contract ở xa 
//k kế thừa thì ta phải viết interface cho nó và gọi theo địa chỉ.
...
//Để ý cách dùng dưới khác cách dùng trên vì khi dùng new với 1 contract or interface thì nó hiểu là đổi state var nên sẽ thành 
//1 transaction mất gas trong khi cách dùng trên thì k. Ta có thể gọi hàm gán new vào 1 hàm transaction duy nhất để gọi nó 
//trc khi dungf để chỉ gọi new 1 lần cho tiết kiệm

//Có thể convert từ address sang string với abi.encodePacked thoải mái như từ string sang address thì k
//interface có thể ké thừa; gọi trực tiếp tên của interface => khi đó nó như 1 hàm số nhận vào địa chỉ của contract để thực th
//các hàm bên trong; tạo ra 1 biến instance của interface đó như ví dụ trên
