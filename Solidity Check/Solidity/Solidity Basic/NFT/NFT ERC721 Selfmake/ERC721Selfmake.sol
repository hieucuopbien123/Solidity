...
//trong OpenZeppelin có nhiều các lib có sẵn cung các tính năng hay vd enumerable cho ta duyệt qua các token.
//Vd ERC721Burnable ta tự implement trong này r.
//Hàm beforeTokenTransfer của ERC-721, ta tự hỏi là để làm gì? Thực chất là khi ta thêm các extensions mở rộng cho 
//contract của ta thì sẽ cần đến hàm này. Vd: thêm enumerable thì phải gắn thêm index vào các hàm để lưu lại trong 1
//mapping mà duyệt. Thế chẳng lẽ viết vào từng hàm 1. K, trước khi transfer, ta lưu lại thông tin vào biến mapping để 
//duyệt, tất cả thao tác trong 1 hàm chung là beforeTokenTransfer
contract MyERC721 is MyContext, MyERC165, IERC721, IERC721Metadata, IERC721Enumerable{
    ...
    function _removeEnumerableWhenBurn(address from, uint tokenId) internal {
        ...
        //thiếu update tokenIndexes và allTokenIndexes của phần tử cuối sau khi đảo lên đầu
    }
    ...
    //IERC721Enumerable
    //giải quyết vấn đề: muốn xóa phần tử có giá trị chỉ định. Rõ ràng ta dùng mapping[địa chỉ sở hữu][index để duyệt]
    //= tokenId => bh muốn xóa 1 tokenId, ta phải duyệt 1 vòng for rất dễ dàng vì forloop solidity ez như C++. Tuy nhiên
    //phần tử càng nhiều thì for loop càng chậm, do đó ta dùng 1 mapping khác lưu ngược lại tokenId=>index thì sẽ ez hơn
    ...
    //burn là ta gửi vào address 0 -> thế thì vẫn mint đc sau khi burn. Ta có thể thêm biến bool để xác định đã burn k thể mint tiếp
    //cx đc
    function burn(uint tokenId) public _onlyOwner(tokenId){
        //chủ của tokenId mới đc burn
    ...
    }/*Cơ chế: to là nơi ta gửi đến. Ta có thể gửi đến 1 account người dùng thì mặc định là nhận được. Nhưng nếu gửi vào 1 địa
    chỉ contract thì phải check xem địa chỉ đó có chịu nhận ERC721 k, vì nhiều ứng dụng mà có khả năng thao tác với NFT 721 thì
    địa chỉ contract nó dùng phải đc implement khă năng nhận ERC-721
    ...
    IERC721Receiver.onERC721Received.selector => Do đó, nó gọi hàm onERC721Received của tại địa chỉ của contract, nếu hàm 
    này có signature như v sẽ return true, nếu chưa implement thì báo lỗi k có hàm, ta catch error đó và revert như trên
