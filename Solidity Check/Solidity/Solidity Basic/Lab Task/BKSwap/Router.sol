...
contract Router is IRouter{
    ...
    //lúc gọi hàm này là đã approve cho contract này dùng tưng đó lượng token r
    ...
    //hàm add nó đã cân cả fee-on-transfer token r. Ta nên ưu tiên làm kiểu lấy balance hơn là truyền tham số vì tiện hơn
    //hàm remove bth là cái pair nó cũng gửi trực tiếp vào địa chỉ r, nên tự hỗ trợ feeontransfer r. Chỉ cần làm với 
    //ETH và permit nx thôi
    //Còn swap thì feeontransfer ta k hỗ trợ hàm đổi ra 1 lượng chính xác fee-on-transfer token vì ta chỉ có 1 cách duy
    //nhất tính lượng token nhận được feeontransfer là lấy cuối trừ đầu mà thôi nên k hỗ trợ. Khi người dùng đổi 1 lượng
    //chính xác fee-on-transfer thì front-end có thể k hiển thị tính năng or chấp nhận hiện thị nhưng đưa ra cảnh báo 
    //người nhận sẽ nhận đc 1 lượng ít hơn mong muốn do fee
    ...
    uint deadline) public override{
        ...
        uint amount1;
        uint amount2;
        {//fix stack too deep error chỉ cho lưu 16 biến ở 1 thời điểm xác định(dùng modifier cũng tốn stack)
