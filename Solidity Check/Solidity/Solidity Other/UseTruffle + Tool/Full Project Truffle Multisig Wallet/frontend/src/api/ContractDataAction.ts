...
export async function getData(web3: Web3, account: string): Promise<GetResponse>{
    MultiSigWallet.setProvider(web3.currentProvider);//ý là provider đã dùng r thì mới gọi đến dây thì nó
    //dùng tiếp cái đó, ở đây là window.ethereum
    ...
    //ví dụ khi viết contract ta tinh ý trong việc viết các hàm get ví dụ ở đây ta có luôn hàm get như này
    //nhờ ta đã định nghĩa trong contract. Nếu k đc thì ta có thể lấy 1 list cái transaction rồi tính length
    //của nó nhưng điều này là tối kỵ vì viết 1 hàm return list các transaction sẽ nguy cơ out of gas
    //còn isConfirmed tương tự, ta chủ đích viết các hàm get cho các thứ ta dùng ở front end
    const count: number = transactionCount.toNumber();
    // const count = transactionCount.toNumber();//phải convert về number
...
//Về cấu trúc file ta có thể tách ra deposit ra file khác vì file này chỉ nên dùng cho data initialization
//nch là tính năng deposit là do hàm ở front end gọi luôn vì có thao tác ấn nút mới thực hiện
...
export function subscribeToEvent(//subscribe trnog web3 có sẵn và nó kp là hàm async
    web3: Web3, 
    address: string,
    callback: (error: Error | null, log: any) => void //chú ý type function có return gì k nhé
    //k được dùng kiểu => { } là buộc có return gì đó
){
    ...
    const res = contractInstance.events.allEvents((error: Error, log: any) => {
        //chú ý: xử lý như này là quá bth vì ta viết ở đây nhưng k muốn hàm này xử lý luôn mà muốn giao
        //cho 1 hàm gọi nó xử lý thì có thể dùng như này, lấy 2 cả 2 thứ là error và data
        // VD hàm xử lý nó phải dùng biến account thì nó có luôn chứ hàm này k có biến account
        if(error) {
    ...
    //nghiên cứu master về cái này cho t: lỗi kinh khủng nhất mất cả ngày mới nghĩ ra
    //hàm useEffect khi return có vai trò rất quan trọng: nó sẽ return những giá trị mà có thể dừng cái hàm
    //vô tận chạy bên trong useEffect hiện tại. Ta hiểu là useEffect nó được gọi nhiều lần tùy vào tham số
    //phụ thuộc hook vào cái gì, nếu mỗi lần gọi nó chạy 1 hàm vô tận mà cứ bị gọi lại trong khi k dừng hàm
    //sẽ cộng dồn gây lag trầm trọng. Nhưng giá trị nó return là gì => bắt buộc phải là 1 function và
    //function đó gọi hàm xóa=> VD: ta return () => res.unsubscribe() thì ok nhưng return res.unsubscribe()
    //thì lỗi ngay vì TH thứ 2 kp khai báo hàm mà có () tức là gọi hàm cmnr và giá trị trả về của vc gọi 
    //hàm được cast sang hàm số cho nó return-> trong quá trình nó gọi hàm thì có thể k tính như ở TH này
    //ta làm sai dãn đến nó bắt event cộng dồn lên mấy lần và in sai numConfirmation
}
...
export async function confirmTrans(web3: Web3, account: string, txIndex: number){
    ...
    console.log("call to datainitialization");
}
