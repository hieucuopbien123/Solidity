...
contract MultisignatureWallet{
    ...
    constructor(address[] memory _owners, uint _numConfirmationRequired) {
        ...
        for(uint i = 0; i < _owners.length; i++){
            ...
            require(!isOwner[owner],"Owner not unique");//check owner k bị trùng nhau
            isOwner[owner] = true;
            owners.push(owner);
        }
    ...
    //cái data truyền vào hàm là kiểu bytes và ta gọi hàm đó về sau tức là ta truyền vào cho nó 1 
    //abi.encodeWithSignature để nó gọi vào hàm nào đó. Nếu k thì phải truyền bytes bất kỳ thì phải đúng form 
    //"0x00","0xaa" or "0xff" k được truyền 0x0 là sai luôn
        ...
        notExecuted(_txIndex)
    {
        ...
        transaction.executed = true;//cản reentrancy r, gọi phát nx sẽ bị cản bởi modifier
        //đầu tiên phải deposit vào trans này, sau đó gọi hàm này để call chuyển tiền từ contract này
        //sang địa chỉ ng nhận
    ...
    //ở phiên bản cũ chỉ dùng 1 fallback function. Bh nó tách ra làm 2 hàm. 1 là fallback(), 2 là receive()
    //receive được gọi khi calldata () là empty bất kể có ether hay k. Còn fallback được gọi khi k có function
    //nào match cả-> nếu k khai báo receive thì nó vẫn gọi vào fallback mà thôi nhưng sẽ throw warning
    //Còn fallback nếu mà k có payable thì công dụng tương tự nhưng k được gọi khi truyền vào ether
    //=> Tức 1 cái là call() empty hoàn toàn còn 1 cái là call với tham số k empty và k match hàm nào cả
